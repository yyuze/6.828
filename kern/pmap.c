...
page_init(void)
{
    ...
    physaddr_t kernel_loaded_pa = 0x100000;
    physaddr_t kernel_end_pa = PADDR((void *)nextfree);
    for (size_t i = EXTPHYSMEM / PGSIZE; i < npages; ++i) {
        struct PageInfo *page = &pages[i];
        physaddr_t page_pa = page2pa(page);
        if (page_pa >= kernel_loaded_pa && page_pa < kernel_end_pa) {
            page->pp_ref = 1;
            page->pp_link = NULL;
        } else {
            page->pp_ref = 0;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    physaddr_t pgtab_pa = PTE_ADDR(*pde);
    uintptr_t pgtab_va = (uintptr_t)KADDR(pgtab_pa);
    pte_t *pgtab = (pte_t *)pgtab_va;
    ret = pgtab + PTX(va);
end:
    return ret;
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
    ...
    size_t acc = 0;
    while (acc < size) {
        uintptr_t mapping_va = va + acc;
        physaddr_t mapping_pa = pa + acc;
        pte_t *pte = pgdir_walk(pgdir, (void *)mapping_va, 1);
        if (pte == NULL) {
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
    ...
    if ((*pte & PTE_P) != 0 && PTE_ADDR(*pte) != pa)
...
check_page_free_list(bool only_low_memory)
{
	...
	struct PageInfo *pp;
	unsigned pdx_limit = only_low_memory ? 1 : NPDENTRIES; /* pdx_limit = 1 */
